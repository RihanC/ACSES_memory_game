...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const db = window.db; // Firestore from index.html
...
function flipCard(card) {
  // Ignore if board is locked OR same card clicked OR card already flipped
  ...
  async function loadLeaderboard() {
    try {
      const q = query(collection(db, "scores"), orderBy("moves", "asc"), limit(5));
